...
export function ResourceHub() {
  ...
  const [hasAutoGenerated, setHasAutoGenerated] = useState<Set<string>>(new Set());

  // Auto-generation logic for 30s viewing
  React.useEffect(() => {
    let timer: NodeJS.Timeout;
    if (selectedVideo && !hasAutoGenerated.has(selectedVideo.id) && !aiNotes) {
      setTimerStart(Date.now());
      timer = setTimeout(() => {
        if (selectedVideo && !aiNotes) {
          handleGenerateAiNotes(selectedVideo);
          setHasAutoGenerated(prev => new Set(prev).add(selectedVideo.id));
        }
      }, 30000); // 30 seconds
  ...
  const handleGenerateAiNotes = async (videoOverride?: Resource) => {
    ...
    try {
      const response = await ai.models.generateContent({
        model: MODELS.flash,
        contents: `Act as a senior engineering consultant. Perform a deep-dive "Source Insight" pass on the following educational resource:
        
        Title: ${video.title}
        Description: ${video.description}
        
        Requirements:
        1. Create a structured technical summary using professional engineering terminology.
        2. Identify specific formulas (e.g. Bernoulli, Cauchy, Maxwell) relevant to the context.
        3. List 3 key practical applications in modern industry (Aero, Bio, or Silicon Valley tech).
        4. Provide a "Future Perspective" note on how AI or Robotics might evolve this specific concept.
        
        Style: Highly descriptive, technically authoritative, and formatted in Markdown with bold headers and bullet points.`,
        config: {
          systemInstruction: "You are an elite technical educator. Your goal is to synthesize complex engineering data into clear, actionable intelligence.",
        }
      });
